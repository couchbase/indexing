...
func NewCodebookIVFSQ(dim, nlist int, sqRange common.ScalarQuantizerRange, metric c.MetricType, useCosine bool) (c.Codebook, error) {
...
func (cb *codebookIVFSQ) ComputeDistanceEncoded(qvec []float32,
	...
	if cb.metric == c.METRIC_L2 {
		...
		if cb.useCosine {
			//faiss.RenormL2(cb.dim, 1, qvec)
			err := cb.index.ComputeDistanceEncoded(qvec, n, codes, dists, nil, listno,
				convertToFaissMetric(cb.metric), cb.dim)
			// Cosine distance is calculated as 1 - (cosine similarity).
			// Cosine similarity ranges from -1 (exactly opposite) to 1 (exactly the same),
			// while cosine distance ranges from 0 (exactly the same) to 2 (exactly opposite).
...
func (cb *codebookIVFSQ) NumCentroids() int {
	return cb.nlist
}

func (cb *codebookIVFSQ) MetricType() c.MetricType {
	return cb.metric
}
