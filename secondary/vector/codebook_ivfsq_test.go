...
var computeDistanceEncodedSQTests = []codebookIVFSQTestCase{

	{"SQ8 L2", 128, cbpkg.METRIC_L2, false, 1, common.SQ_8BIT, 10000, 10000},
	{"SQ8 DOT", 128, cbpkg.METRIC_INNER_PRODUCT, false, 1, common.SQ_8BIT, 10000, 10000},
	{"SQ8 COSINE", 128, cbpkg.METRIC_INNER_PRODUCT, true, 1, common.SQ_8BIT, 10000, 10000},
}

func TestComputeDistanceEncodedSQ(t *testing.T) {
	seed := time.Now().UnixNano()
	for _, tc := range computeDistanceEncodedSQTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			t0 = time.Now()
			err = codebook.ComputeDistanceEncoded(qvec, n, icodes, dist2, nil, listno)
			if err != nil {
				t.Errorf("Error computing encoded distance %v", err)
			}
			delta = time.Now().Sub(t0)
