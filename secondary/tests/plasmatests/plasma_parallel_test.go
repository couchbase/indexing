...
func createSlice1(c *Context) {
	...
	common.CrashOnError(err1)
	c.memDbSlice = slice1
	slice, err := indexer.NewPlasmaSlice(c.plasmaDir, c.plasmaDir, "", 0, idxDefn, instID, common.PartitionId(0), false, 1, config, stats, int64(config.GetIndexerMemoryQuota()), false, true, nil, 64, 0, nil, nil, nil)
