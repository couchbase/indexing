...
func createSlice1(c *Context) {
	...
	slice1, err1 := indexer.NewMemDBSlice(c.memDbDir, 0, idxDefn1, instID, common.PartitionId(0), false, true, 1, config, stats, 1024, indexer.EncrCbsTest)
	common.CrashOnError(err1)
	c.memDbSlice = slice1
	slice, err := indexer.NewPlasmaSlice(c.plasmaDir, c.plasmaDir, "", 0, idxDefn, instID, common.PartitionId(0), false, 1, config, stats, int64(config.GetIndexerMemoryQuota()), false, true, nil, 64, 0, nil, nil, nil, indexer.EncrCbsTest)
...
func insertDelete1(c *Context, mutationMeta *indexer.MutationMeta, wg *sync.WaitGroup) {
	...
	for i := 0; i < c.numItems; i++ {
		...
		c.memDbSlice.Insert(c.keys[i], c.docid[i], nil, nil, nil, mutationMeta)
		c.plasmaSlice.Insert(c.keys[i], c.docid[i], nil, nil, nil, mutationMeta)
	}
