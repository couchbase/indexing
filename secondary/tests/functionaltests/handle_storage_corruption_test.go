...
func TestBasicSanityMultipleIndices(t *testing.T) {
	...
	scanResults, err = secondaryindex.Range("corrupt_idx2_company", "default", indexScanAddress, []interface{}{"G"},
		[]interface{}{"M"}, 1, false, defaultlimit, c.SessionConsistency, nil)
	FailTestIfError(err, "Error in range scan", t)
	err = tv.Validate(docScanResults, scanResults)
	FailTestIfError(err, "Error in scan result validation", t)

	scanResults, err = secondaryindex.Range("corrupt_idx1_age", "default", indexScanAddress, []interface{}{35},
...
func TestPartitionedIndex(t *testing.T) {
	...
	scanResults, err = secondaryindex.Range("corrupt_idx3_age", "default", indexScanAddress,
		[]interface{}{21}, []interface{}{21}, 3, false, defaultlimit, c.SessionConsistency, nil)
	FailTestIfError(err, "Error in scan of corrupt index", t)

	if len(scanResults) != 0 {
		err = errors.New("Unexpected number of results in index scan")
		FailTestIfError(err, "Error in scan of corrupt index", t)
	}
...
func TestMOITwoSnapsOneCorrupt(t *testing.T) {
...
func TestMOITwoSnapsBothCorrupt(t *testing.T) {
	if clusterconfig.IndexUsing != "memory_optimized" {
	...
	// Step 1: Create an index
	fmt.Println("Creating an index ...")
	err = secondaryindex.CreateSecondaryIndex("corrupt_idx5_age", "default", indexManagementAddress,
		"", []string{"age"}, false, nil, false, 60, nil)
	FailTestIfError(err, "Error in creating the index", t)
	...
	var slicePath string
	slicePath, err = tc.GetIndexSlicePath("corrupt_idx5_age", "default", absIndexStorageDir, 0)
	FailTestIfError(err, "Error in GetIndexSlicePath", t)
	...
	// Step 5: Query on the index should not fail.
	_, errRange := secondaryindex.Range("corrupt_idx5_age", "default", indexScanAddress, []interface{}{35},
		[]interface{}{40}, 1, false, defaultlimit, c.SessionConsistency, nil)
	if errRange == nil {
		err = errors.New("Unexpected success of scan as index was corrupted")
		FailTestIfError(err, "Error in scan of corrupt index", t)
