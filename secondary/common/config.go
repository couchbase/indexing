...
var SystemConfig = Config{
	...
	"indexer.bhive.vanama.efSearch": ConfigValue{
		64,
		"number of nodes to visit during search",
		64,
		false,
		false,
	},
	...
	"indexer.bhive.vanama.efPruneThreshold": ConfigValue{
		0.2,
		"threshold for keeping long edges in graph",
		0.2,
		false,
		false,
	},
	"indexer.bhive.vanama.dualExpansionRatio": ConfigValue{
		0.5,
		"ratio between W (cosine) and R (IP) expansion.  Lower value means more W expansion.",
		0.5,
		false,
		false,
	},
	"indexer.bhive.vanama.dualBatchFlushSize": ConfigValue{
		8,
		"Dual-expansion scoring-batch flush gate: number of unseen neighbors buffered before a SIMD distance-comparison flush. Multiple of 8 recommended (SIMD kernel width); smaller keeps the lazy-R frontier fresher, larger amortizes per-flush overhead. Floored to 8.",
		8,
		false,
		false,
	},
	"indexer.bhive.vanama.maxSeeds": ConfigValue{
		384,
		"Number of vectors to use for initial seed selection",
		384,
		false,
		false,
	},
	"indexer.bhive.vanama.sparse.expansionSeeds": ConfigValue{
		32,
		"Number of vectors to use for graph expansion",
		32,
		false,
		false,
	},
	"indexer.bhive.vanama.sparse.seedTrimMassFraction": ConfigValue{
		0.9,
		"Cumulative weight fraction to determine each dim's trimmed posting list in the seed table; the trimmed list is the shortest weight-descending prefix whose summed weight >= SparseSeedTrimMassFraction * total_weight_at_dim. Lower = more aggressive trim. Range (0, 1]",
		0.9,
		false,
		false,
	},
	"indexer.bhive.query.pruneMaxDims": ConfigValue{
		10,
		"Caps the number of query coordinates used during a bhive sparse scan: the query is trimmed to the QueryPruneMaxDims entries with the largest |weight| (concentration of importance) before both the per-centroid dist-comp kernels and the common-inverted-index BMW scan, which scale with the query dim count. Preserves nearly all of the inner product while cutting scan CPU. 0 (default) disables pruning (full query). Query-time only; no rebuild needed",
		10,
		false,
		false,
	},
	"indexer.bhive.vector.pruneMaxDims": ConfigValue{
		30,
		"Caps the number of coordinates stored per bhive document vector. At ingestion, before the vector is persisted and the graph is built, it is trimmed to the VectorPruneMaxDims entries with the largest |weight| (concentration of importance). The norm cache, quantized codes, Vamana graph, per-cell seed tables, and common inverted index all derive from the pruned vector, cutting resident memory and dist-comp work; denoising the cosine direction can also improve graph neighbor quality. 0 (default) disables pruning (full vector). Build-time: only affects vectors ingested after the change. For SPLADE, ~75 dims preserves nearly all of the inner product, ~50 preserves ~97%",
		30,
		false,
		false,
	},
	...
	"indexer.bhive.invertedIndex.blockSize": ConfigValue{
		32,
		"BMW (Block-Max MaxScore) block size in entries for the common inverted index; smaller blocks give tighter per-block upper bounds",
		32,
		false,
		false,
	},
	"indexer.bhive.invertedIndex.topN": ConfigValue{
		384,
		"Heap size for the BMW traversal over the common inverted index; 0 disables the slice-wide common-inverted-index path entirely (Build is skipped and the sentinel-centroid scan is a no-op)",
		384,
		false,
		false,
	},
	"indexer.bhive.invertedIndex.candidateMultiplier": ConfigValue{
		6,
		"Sizes the BMW candidate pool for a sentinel-centroid scan: topN = candidateMultiplier * max(topNScan, limit). Headroom for the heuristic-UB candidate selection and downstream dedup; deeper pools improve recall at some scan cost. Floored to 4 when <=0",
		6,
		false,
		false,
	},
	...
	"indexer.bhive.numEvictor": ConfigValue{
		1,
		"number of evictor per shard",
		1,
		false,
		false,
	},
	"indexer.bhive.numInitBuilder": ConfigValue{
		runtime.GOMAXPROCS(0),
		"number of builder for initial index build (per index)",
		runtime.GOMAXPROCS(0),
		false,
		false,
	},
	"indexer.bhive.numBuilder": ConfigValue{
		runtime.GOMAXPROCS(0) / 8,
		"number of builder for graph rebuild (per index)",
		runtime.GOMAXPROCS(0) / 8,
		false,
		false,
	},
	...
	"indexer.bhive.LSSCleanerThreshold": ConfigValue{
		30.0,
		"The target fragmentation percent the log cleaner should maintain.",
		30.0,
		false,
		false,
	},
	...
	"indexer.bhive.enableCompactFullMarshal": ConfigValue{
		true,
		"compact cell upon full marshal if it exceeds the purge thresholds",
		true,
		false,
		false,
	},
	"indexer.bhive.purgeThresholdMax": ConfigValue{
		float32(8),
		"It is used to compact a cell with a rollback if the instance purge ratio exceeds the threshold",
		float32(8),
		false,
		false,
	},
	"indexer.bhive.purgeThresholdMin": ConfigValue{
		float32(4),
		"It is used to compact a cell whose local purge ratio exceeds the threshold. Also compactor starts scanning when instance purge ratio exceeds the threshold",
		float32(4),
		false,
		false,
	},
	"indexer.bhive.purgeThresholdReader": ConfigValue{
		float32(4),
		"reader purge threshold to set the purge hint during a cell scan. The hint is used to compact either during a flush or by mvcc compactor",
		float32(4),
		false,
		false,
	},
	"indexer.bhive.purgeThresholdRecords": ConfigValue{
		1000,
		"minimum number of records in a cell to be eligible for compaction",
		1000,
		false,
		false,
	},
	"indexer.bhive.compactor.enable": ConfigValue{
		true,
		"enable background compactor per instance to contain mvcc purge ratio (enableCompactFullMarshal should also be enabled)",
		true,
		false,
		false,
	},
	"indexer.bhive.compactor.diskLimit": ConfigValue{
		25,
		"avail disk space percent above which compactor will not act even on exceeding the max purge threshold",
		25,
		false,
		false,
	},
	"indexer.bhive.compactor.interval": ConfigValue{
		10,
		"time period in minutes for compactor to check for instance mvcc purge ratio",
		10,
		false,
		false,
	},
	"indexer.bhive.compactor.rateLimit": ConfigValue{
		1000,
		"rate limit number of cell compactions per second for all instances in a lss vstore",
		1000,
		false,
		false,
	},
	"indexer.bhive.compactor.batchLimit": ConfigValue{
		10000,
		"controls number of cells compacted in a compaction cycle for an instance",
		10000,
		false,
		false,
	},
	...
	"indexer.bhive.throttleMinFragThreshold": ConfigValue{
		80.0,
		"Minimum fragmentation after which writer throttling is enabled",
		80.0,
		false,
		false,
	},
	"indexer.bhive.throttleMinSize": ConfigValue{
		1024 * 1024 * 1024,
		"Minimum used space in bytes after which writer throttling is enabled",
		1024 * 1024 * 1024,
		false,
		false,
	},
	"indexer.bhive.throttleDiskLimit": ConfigValue{
		10,
		"Disk space available in percent after which writer throttling is enabled",
		10,
		false,
		false,
	},
	"indexer.bhive.monitorTimerDuration": ConfigValue{
		1,
		"Duration of the timer for monitoring the disk space",
		1,
		false,
		false,
	},
	"indexer.bhive.maxThrottleDuration": ConfigValue{
		5,
		"Maximum duration for which writer throttling is enabled in a single transaction. Defaults to 5mins",
		5,
		false,
		false,
	},
