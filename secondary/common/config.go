...
var SystemConfig = Config{
	...
	"indexer.bhive.vector.pruneMaxDims": ConfigValue{
		100,
		"Caps the number of coordinates stored per bhive document vector. At ingestion, before the vector is quantized and the graph is built, it is trimmed to this many entries with the largest |weight| (concentration of importance). The quantized codes, Vamana graph, per-cell seed tables, and common inverted index all derive from the pruned vector, cutting resident memory and dist-comp work; denoising the cosine direction can also improve graph neighbor quality. 0 disables pruning (full vector); vectors with fewer coordinates are stored unchanged. Build-time: only affects vectors ingested after the change. For SPLADE, ~75 dims preserves nearly all of the inner product, ~50 preserves ~97%",
		100,
		false,
		false,
	},
