...
func (ivc *internalVersionChecker) getNodes() *nodeList {
	...
	for _, svc := range ivc.svcs {
		...
		case "index":
			...
			populate(ivc.cinfo.GetNodesByServiceType(INDEX_HTTP_SERVICE), nodes.idxs)
			if ivc.checkFailedNodes {
				populate(ivc.cinfo.GetFailedIndexerNodes(), nodes.fidxs)
			}

		case "kv":
			populate(ivc.cinfo.GetNodesByServiceType(KV_SERVICE), nodes.kvs)
			if ivc.checkFailedNodes {
				populate(ivc.cinfo.GetFailedKVNodes(), nodes.fkvs)
			}

		case "n1ql":
			populate(ivc.cinfo.GetNodesByServiceType(CBQ_SERVICE), nodes.n1qls)
			if ivc.checkFailedNodes {
				populate(ivc.cinfo.GetFailedQueryNodes(), nodes.fn1qls)
			}
		}
	}
...
func (ivc *internalVersionChecker) getNodeVersion(node couchbase.Node, svc string) {
	...
	nid, found := ivc.cinfo.GetNodeIdByUUID(node.NodeUUID)
	if !found {
	...
	addr, err := ivc.cinfo.GetServiceAddress(nid, ivc.svcPortMap[svc], true)
	if err != nil {
...
func (mon *internalVersionMonitor) getVersion() InternalVersion {
	...
	err := mon.cinfo.FetchNodesAndSvsInfoWithLock()
	if err != nil {
	...
	ivc := newInternalVersionChecker(mon.cinfo, []string{"index", "kv", "n1ql"}, true, mon.cache)
...
//
func GetInternalClusterVersion(cinfo *ClusterInfoCache, checkFailedNodes bool) (InternalVersion, error) {

	ivc := newInternalVersionChecker(cinfo, []string{"index", "kv", "n1ql"}, checkFailedNodes, nil)
...
//
func GetInternalIndexerVersion(cinfo *ClusterInfoCache, checkFailedNodes bool) (InternalVersion, error) {

	ivc := newInternalVersionChecker(cinfo, []string{"index"}, checkFailedNodes, nil)
	return ivc.getVersion()
}
...
func MonitorInternalVersion(
	...
	clusterUrl, err := ClusterAuthUrl(clusterAddr)
	if err != nil {
		logging.Fatalf("MonitorInternalVersion error in ClusterAuthUrl %v", err)
		return
	}

	cinfo, err := NewClusterInfoCache(clusterUrl, DEFAULT_POOL)
	if err != nil {
		logging.Fatalf("MonitorInternalVersion error in NewClusterInfoCache %v", err)
		return
	}
	...
	func() {
		...
		cinfo.SetUserAgent("MonitorInternalVersion")
	}()

	mon := newInternalVersionMonitor(cinfo, termVer, termIntVer, clusterAddr)
