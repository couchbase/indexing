...
func (codec *Codec) DecodeN1QLValue(code, buf []byte) (val n1ql.Value, err error) {
	...
	val, _, err = codec.code2n1ql(code, buf)
	return val, err
}

func (codec *Codec) code2n1ql(code, text []byte) (n1ql.Value, []byte, error) {
	if len(code) == 0 {
		return nil, nil, nil
	}
		...
		if codec.arrayLenPrefix {
			...
			if err == nil {
				for ; l > 0; l-- {
					ln := len(text)
					lnc := len(code)
					tv, code, err = codec.code2n1ql(code, text[ln:])
					if err != nil {
						break
					}
		...
		if codec.propertyLenPrefix {
			...
			if err == nil {
				for ; l > 0; l-- {
					...
					ln := len(text)
					lnc := len(code)
					key, code, err = codec.code2n1ql(code, text[ln:])
					if err != nil {
						break
					}
					text = text[:ln+lnc-len(code)]
					...
					ln = len(text)
					lnc = len(code)
					value, code, err = codec.code2n1ql(code, text[ln:])
					if err != nil {
						break
					}
					text = text[:ln+lnc-len(code)]
