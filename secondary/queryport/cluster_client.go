...
func (c *ClusterClient) CreateIndex(
	name, bucket, using, partnExpr, whereExpr string,
	...
	if err == nil { // Post HTTP request.
		...
		if err == nil {
			...
			if mresp, err := c.metaResponse(resp); err == nil {
...
// DropIndex `DefnID` identies the index to be droped but is unique-id.
func (c *ClusterClient) DropIndex(defnID string) error {
	...
	if err == nil {
		...
		if err == nil {
			defer resp.Body.Close()
			if _, err := c.metaResponse(resp); err == nil {
				return nil
			}
			return err
		}
