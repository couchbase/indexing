...
func TestConnPoolBasicSanity(t *testing.T) {
	...
	cp := newConnectionPool(host, 3, 6, 1024*1024, readDeadline, writeDeadline, 3, 1)
...
func TestConnRelease(t *testing.T) {
	...
	cp := newConnectionPool(host, 500, 10, 1024*1024, readDeadline, writeDeadline, 40, 10)
	cp.mkConn = testMkConn
...
func TestLowWM(t *testing.T) {
	...
	cp := newConnectionPool(host, 20, 5, 1024*1024, readDeadline, writeDeadline, 10, 2)
	cp.mkConn = testMkConn
...
func TestTotalConns(t *testing.T) {
	...
	cp := newConnectionPool(host, 120, 5, 1024*1024, readDeadline, writeDeadline, 10, 10)
	cp.mkConn = testMkConn
...
func TestUpdateTickRate(t *testing.T) {
	...
	cp := newConnectionPool(host, 40, 5, 1024*1024, readDeadline, writeDeadline, 2, 2)
	cp.mkConn = testMkConn
