...
**DCP** - Universal Protocol for Replication aims to connect different parts
...
master that has started hosting the vbucket. Refer to DCP documents for more
details.
...
sequence number due to de-duplication. Refer to DCP documentation for more.
...
events observed by an indexer node. A particular DCP mutation may only be sent
...
posted to projector, to re-connect with KV-cluster for newer set of DCP
...
local-indexer-node. System-wide DCP mutations will be gathered in this queue
...
**KV-reconnection** - A relative harmless scenario when one or more DCP
