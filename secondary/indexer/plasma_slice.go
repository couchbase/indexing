...
func encryptCodebookByChunk(codebookPath, tmpEncryptionPath string, eCtx EncryptionCtx) error {
	...
	cfw, err := NewCryptFileWriter(fd, eCtx, cryptFileWriterReaderBufSz, aligned, nil)
	if err != nil {
...
func (mdb *plasmaSlice) Statistics(consumerFilter uint64) (StorageStatistics, error) {
	...
	mdb.idxStats.rCacheHits.Set(rCacheHits)
	mdb.idxStats.rCacheMisses.Set(rCacheMisses)
...
func updatePlasmaConfig(cfg common.Config) {
	...
	plasma.SetDropKeyRateLimit(int64(cfg["plasma.encryption.LSSCleanerDropKeyRateLimit"].Int()) * 1024 * 1024)
	plasma.SetDropKeyConcurrency(cfg["plasma.encryption.LSSCleanerDropKeyMaxCpu"].Float64(),
		cfg["plasma.LSSCleanerConcurrency"].Int())
}
...
func (mdb *plasmaSlice) BuildDone(idxInstId common.IndexInstId, callb BuildDoneCallback) {
...
func decryptCodebookByChunk(newFilePath string, getKeyById func(keyid []byte) []byte, codebookKDFLabelCtx []byte) ([]byte, error) {
	...
	_, err := DecryptFileByChunk(context.Background(), newFilePath, decryptedFile, getKeyById, codebookKDFLabelCtx, nil)
	if err != nil {
