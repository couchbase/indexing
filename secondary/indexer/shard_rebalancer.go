	...
	for _, idxInst := range tt.IndexInsts {
		...
		if idxInst.Defn.IsVectorIndex {
			for _, partnId := range idxInst.Defn.Partitions {
				...
				// For replica repair (Copy) the codebook lives on the sibling index node; for
				// upgrade moves (Move) the source node may be v1-compat with old-format paths.
				// In both cases InstRenameMap carries the authoritative source path.
				for _, renameMap := range tt.InstRenameMap {
					currCodebookPath, _ := generateCodebookRenamePaths2(
						renameMap,
						idxInst.Defn.Bucket, idxInst.Defn.Name, idxInst.Defn.BucketUUID,
...
func generateCodebookRenamePaths2(
	...
	v2Prefix := fmt.Sprintf("%s_", bucketUUID)
	v1Prefix := fmt.Sprintf("%s_%s_", bucket, name)

	for currPath, newPath := range renameMap {
		// v2 source → v2 dest
		if trimmedCurrPath, ok := strings.CutPrefix(currPath, v2Prefix); ok {
			currInstId, newInstId, extractedPartnId, err := extractIndexInfoFromRenamePath(
				trimmedCurrPath, strings.TrimPrefix(newPath, v2Prefix),
			)
			if err != nil || extractedPartnId != partnId || newInstId != instId {
				continue
			}
			...
			return filepath.Join(currIndexPath, CODEBOOK_DIR, fmt.Sprintf("%d_%d.codebook", currInstId, partnId)),
				filepath.Join(newIndexPath, CODEBOOK_DIR, fmt.Sprintf("%d_%d.codebook", newInstId, partnId))
		}

		// v1 source → v2 dest (upgrade path)
		if trimmedCurrPath, ok := strings.CutPrefix(currPath, v1Prefix); ok {
			var currInstId common.IndexInstId
			var extractedPartnId common.PartitionId
