...
func (m *CommandListener) handleNewDropInstanceCommandToken(path string, value []byte) {
	...
	if err != nil {
		logging.Errorf("LifecycleMgr.GetLatestReplicaCount(): Fail to convert last part %v to int: %v", chunkIndexStr, err)
		return
	}
	if chunkIndex != 0 {
		return
	...
	if err != nil {
		logging.Errorf("LifecycleMgr.GetLatestReplicaCount(): Fail to retrieve drop instance token for path %v got err: %v", path, err)
		return
	}
